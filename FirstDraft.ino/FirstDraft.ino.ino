...
void loop() {
  if(sSerial.available() > 0) {
    data = sSerial.read();
    if (data != 1){} //since the first byte read is a status code where 1 means the read was successful if the data is anything other than that the data is corruoted and useless 
